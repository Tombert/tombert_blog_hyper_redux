...
So  [[Posts/Technical/2025/03 March/Recent Projects - State Store\|last time]]  I described how handled the `put` logic in the state store replacement in Kafka Streams.  If you haven't read that, then this post is probably going to be confusing, so you should probably read that first. 
